...
const SETORES_VALIDOS = ['FRIOS', 'SECOS'];

const createUserSchema = Joi.object({
    nome: Joi.string().min(3).max(120).required().label('Nome'),
    email: Joi.string()
        .email({ tlds: { allow: false } })
        .max(180).required().label('E-mail'),
    senha: Joi.string()
        .pattern(SENHA_REGEX).required().label('Senha')
        .messages({
            'string.pattern.base':
                'Senha deve ter no mínimo 8 caracteres, 1 letra maiúscula, ' +
                '1 minúscula, 1 número e 1 caractere especial (@$!%*?&#_-).',
        }),
    cargo: Joi.string().valid(...CARGOS_VALIDOS).required().label('Cargo'),
    setor: Joi.string().valid(...SETORES_VALIDOS)
        .allow(null).optional().label('Setor'),
}).messages(messages);

const updateUserSchema = Joi.object({
    nome:  Joi.string().min(3).max(120).label('Nome'),
    email: Joi.string()
        .email({ tlds: { allow: false } }).max(180).label('E-mail'),
    cargo: Joi.string().valid(...CARGOS_VALIDOS).label('Cargo'),
...
const changePasswordSchema = Joi.object({
    senha_atual: Joi.string().required().label('Senha atual'),
    nova_senha: Joi.string()
        .pattern(SENHA_REGEX).required().label('Nova senha')
        .messages({
            'string.pattern.base':
                'Nova senha deve ter no mínimo 8 caracteres, 1 letra maiúscula, ' +
                '1 minúscula, 1 número e 1 caractere especial (@$!%*?&#_-).',
        }),
    confirmar_senha: Joi.string()
        .valid(Joi.ref('nova_senha')).required().label('Confirmar senha')
...
const listUsersSchema = Joi.object({
    ...
    setor:    Joi.string().valid(...SETORES_VALIDOS).optional().label('Setor'),
    ...
    order:    Joi.string().valid('asc','desc').default('asc').label('Direção'),
}).messages(messages);
